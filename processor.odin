...
handle_file :: proc(filename: os.File_Info, directory: string, array: ^[dynamic]Post) {
	...
	base := filepath.base(filename.fullpath)
	only_filename := slugify(strings.trim_suffix(base, ".md"))

	new_file_path, newfile_err := filepath.join({directory, only_filename}, context.temp_allocator)
	if newfile_err != nil {
		fmt.eprintln("Error creating new file path:", newfile_err)
		return
	}
	new_file_with_ext := fmt.tprintf("%s.html", new_file_path)

	// Getting frontmatter and data without frontmatter
	frontmatter, data_without_frontmatter := parse_frontmatter(
		filename,
	...
	fmt.printfln("[Parsing] file %s", filename.fullpath)
	...
	template_file, templ_file_err := filepath.join(
		{"templates", "base.html"},
		context.temp_allocator,
	)
	if templ_file_err != nil {
		fmt.eprintln("Error getting template file:", templ_file_err)
		return
	}
	// template := string(read_file("templates/base.html"))
	template := string(read_file(template_file))
	final_html := apply_template(template, frontmatter, string(html))

	// Writing html contents to file
	fmt.printfln("[Writing] to output public: %s", new_file_with_ext)
	write_err := os.write_entire_file_from_string(new_file_with_ext, string(final_html))
	if write_err != nil {
...
handle_index :: proc(posts: string) {
	...
	template_file_path, tmpl_file_err := filepath.join(
		{"templates", "index.html"},
		context.temp_allocator,
	)
	if tmpl_file_err != nil {
		fmt.eprintln("Error getting template file path:", tmpl_file_err)
		return
	}
	template := string(read_file(template_file_path))

	html, _ := strings.replace_all(template, "{{post_list}}", posts, context.temp_allocator)

	fmt.printfln("[Writing] %s into public/", "index.html")

	index_path, index_err := filepath.join({"public", "index.html"}, context.temp_allocator)
	if index_err != nil {
		fmt.eprintln("Error getting index html file path:", index_err)
		return
	}
	write_err := os.write_entire_file_from_string(index_path, html)
	if write_err != nil {
...
build_post_list :: proc(posts: []Post) -> string {
	...
	for post in posts {
		fmt.sbprintf(
			&b,
			`<li><a href="%s">%s</a> <span class="byline">%s</span></li>`,
			post.url,
			post.title,
			post.date,
		)
